aT = efrfBrierScores.Properties.CustomProperties.agg_times;
aTq = length( aT );
q2t = efrfBrierScores.Properties.CustomProperties.quant2test;
q2tq = length( q2t );
...
scoreF = nan(aTq,q2tq);
for idx = 1:q2tq
    scoreF(:,idx) = cat(2 ,efrfBrierScores{idx, "Fuentes"}{1}(:,1).bs);
end
for idx = 1:aTq
    plot( q2t, scoreF(idx,:), 'LineWidth', 2, 'Color', bGrad(idx, aTq) );
end
for idx = 1:q2tq
    scoreF(:,idx) = cat(2 ,efrfBrierScores{idx, "LakeComo"}{1}(:,1).bs);
end
for idx = 1:aTq
...
for idx = q2tq+(1:q2tq)
    scoreF(idx-q2tq, :, :) = cat(1 ,efrfBrierScores{idx, "Fuentes"}{1}(:,1).bs);
end
...
for idx = q2tq*2+(1:q2tq)
    scoreF(idx-2*q2tq, :, :) = cat(1 ,efrfBrierScores{idx, "Fuentes"}{1}(:,1).bs);
end
scoreLC = nan(q2tq, aTq, 12);
for idx = q2tq*2+(1:q2tq)
    scoreLC(idx-2*q2tq, :, :) = cat(1 ,efrfBrierScores{idx, "LakeComo"}{1}(:,1).bs);
end
tiledlayout(4,3);
for mo = 1:12
    nexttile;
