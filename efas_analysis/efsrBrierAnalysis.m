...
efsrBrierScores = table('Size', [length(scoresnames), length(signalsnames)], ...
...
for idx = 1:length( scoresnames )
    for jdx = 1:length(signalsnames)
        efsrBrierScores{idx,jdx}{1} = repmat( brier_score.empty, length(aT), 1);
    end
end
efsrBrierScores = addprop(efsrBrierScores, 'agg_times', 'table');
efsrBrierScores.Properties.CustomProperties.agg_times = aT;
efsrBrierScores = addprop(efsrBrierScores, 'quant2test', 'table');
efsrBrierScores.Properties.CustomProperties.quant2test = quant2test;
...
for aggT = 1:length(aT)
    ...
    for stg =  1:length( bs_settings)
        ...
        for ref = 5:length(signalsnames)
            ...
            efsrBrierScores{bs_settings(stg).name, signalsnames(ref)}{1}(aggT, 1) = p;
        end
