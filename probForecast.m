    ...
    methods
        ...
        function outputArg = aggregate( obj, agg_time, varargin)
            ...
            elseif strcmp(p.Results.Which, valid_types{2} )
                ...
                outputArg = cell( 1, obj.ensambleN);
                for idx = 1:obj.ensambleN
        ...
        function outputArg = compress( obj, varargin )
            ...
            dummy = zeros( obj.daysN, obj.ensambleN );
            vn = cell(1, obj.ensambleN );
            for idx = 1:obj.ensambleN
                temp = obj.data{idx};
        ...
        function outputArg = max_lead_time( obj, agg_time )
            ...
            if lead_time < 31
                outputArg = min( floor((obj.leadTime-ceil(31/lead_time)*lead_time)/lead_time +1)...
                    , 7);
            else
...
%         function outputArg = probForecast2array ( obj )
%             dummy = obj.data;
%             outputArg = zeros( length(dummy{1}.Time), obj.ensambleN );
%             for idx = 1:obj.ensambleN
%                 temp = table2array( timetable2table( ...
%                     dummy{idx}, 'ConvertRowTimes', false ) );
%                 outputArg(:, idx) = temp(:, 1);
%             end
%         end
    end
    ...
    methods (Access = private)
        ...
        function outputArg = aggregate_(obj, forecast, agg_time )
            ...
            %get the maximum lead time, at most 7..
            lead_time = obj.max_lead_time( agg_time );
            ...
            for idx = 1:length( forecast.Time )-1
                ...
                extracted_values = table2array( timetable2table(...
                    forecast(idx, 1:ne ), 'ConvertRowTimes', false ) );
                extracV_lt = table2array( timetable2table(...
                    forecast(idx, (ne+1):(ne+agg_timeStep*(lead_time-1)) ), 'ConvertRowTimes', false ) );
            ...
            extracted_values = table2array( timetable2table(...
                forecast(end, 1:agg_timeStep*lead_time ), 'ConvertRowTimes', false ) );
